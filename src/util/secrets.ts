...
function parse_env_int(key: string) {
    ...
    if (val && !Number.isNaN(Number.parseInt(val))) {
        ...
        appLogger.error(
            `Please define a valid number value for ${key} in .env`
        );
        process.exit(1);
    }
    return num;
...
export const FE_FULL_URL = process.env["FE_FULL_URL"]
    ? (process.env["FE_FULL_URL"] as string)
    : "";

export const ALLOWED_ORIGINS = [
    BASEURL,
    FE_FULL_URL,
    `${BASEURL}:${PROXY_LISTEN_PORT}`,
];
...
if (
    ...
    appLogger.warn(
        "TLS Certificate information not found or incomplete, app will run in HTTP mode."
    );
}
if (
    TLS_CERT_INFO &&
    TLS_CERT_INFO.type &&
    !["pem", "pfx"].includes(TLS_CERT_INFO.type)
) {
